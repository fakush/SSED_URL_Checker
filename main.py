...
fileName = 'pasadaLote1_conVPN.txt'

#Encabezado del Archivo de texto
header = 'Lista URLS: Pasada Lote1 - sin VPN'
...
def getWebImg(url, counter, prefix):
    ...
    nameString0 = url.replace('.', '-')
    nameString = nameString0.replace('/', '-')
    ...
    try:
        ...
        time.sleep(1)
        #.get_screenshot_as_file('/ProcesosTemp/' + str(counter + 1) + "-" + vpn + "-" + prefix + nameString + '_screenshot.png')
    except:
        print("pasando de largo")
    finally:
        im1 = pyautogui.screenshot()
        im1.save('/ProcesosTemp/' + str(counter + 1) + "-" + vpn + "-" + prefix + nameString + '_fullScreenshot.png')
        # element = driver.find_element_by_class_name("header--navbar")
        # element.screenshot(-r'c:/ProcesosTemp/' + str(counter + 1) + "-" + vpn + "-" + prefix + nameString + '_fullScreenshot.png' % counter)
        driver.quit()
        print("end...")
...
with open(fileName, 'w') as f:
    ...
    for each in listUrls:
        ...
        getWebImg('www.'+listUrls[count], count, "www-")
        try:
